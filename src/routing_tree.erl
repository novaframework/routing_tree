...
-export([new/0, new/1, insert/5, lookup/4]).
...
new(Options) ->
    #host_tree{options =
                   #{use_strict => maps:get(use_strict, Options, false),
...
-spec lookup_path(Segments :: [string()] | binary(),
                  Comparator :: any(),
                  Tree :: [tree_node()]) ->
                     {ok, Bindings :: map(), Value :: any()} |
                     {ok, Bindings :: map(), Value :: any(), PathInfo :: [binary()]} |
...
-spec lookup_path([Segments :: list()],
                  Comparator :: any(),
                  Tree :: [tree_node()],
                  {Bindings :: map(), Node :: tree_node() | undefined}) ->
                     {ok, Bindings :: map(), Value :: any()} | {error, not_found}.
...
lookup_binary(<<>>, Comparator, Tree, {Bindings, _Node}, Ack) ->
    case lookup_segment(Ack, Bindings, Tree) of
        {ok, Bindings0, #node{is_wildcard = false, value = Value}} ->
...
-spec insert(Host :: any(),
             ...
             HT :: host_tree()) ->
                host_tree().
insert(Host, StatusCode, _Comparator, Value, #host_tree{hosts = Hosts} = HT)
    when is_integer(StatusCode) ->
    %% We don't need to tokenize since it's a integer
    RT = case lists:keyfind(Host, 1, Hosts) of
             false ->
                 #routing_tree{};
             {_, RoutingTree} ->
                 RoutingTree
    ...
    case lists:keyfind(StatusCode, #node.segment, RT#routing_tree.tree) of
        false ->
            ...
            RT0 = RT#routing_tree{tree =
                                      [#node{segment = StatusCode,
                                             value = [#node_comp{comparator = '_', value = Value}]}
...
insert(Host,
       ...
       Comparator,
       Value,
       #host_tree{hosts = Hosts, options = Options} = HT) ->
    Tokens =
        case routing_tree_leex:string(Path) of
            {ok, Tokens0, _} ->
                Tokens0;
    ...
    SyntaxTree =
        case routing_tree_parser:parse(Tokens) of
            {ok, STree} ->
                STree;
            {error, {_Line0, _Mod0, Message}} ->
                throw({bad_routingfile, lists:concat(Message)})
        end,
    CompNode = #node_comp{comparator = Comparator, value = Value},
    RT = case lists:keyfind(Host, 1, Hosts) of
             false ->
                 #routing_tree{};
             {_, RoutingTree} ->
...
-spec insert([{Type :: atom(), Line :: number(), Ident :: list()}],
             CompNode :: node_comp(),
             Siblings :: [tree_node()],
             Options :: options()) ->
                Tree :: [tree_node()].
insert([], _CompNode, Tree, _Options) ->
    Tree;
insert([{Type, _, Ident} | Tl],
       CompNode,
       Siblings,
       Options = #{use_strict := UseStrict}) ->
    case UseStrict of
        true ->
    ...
    case lists:keyfind(value(Ident, Options), #node.segment, Siblings) of
        false ->
            ...
            case Tl of
                [] ->
                    [#node{segment = value(Ident, Options),
                           is_binding = Type == binding,
                           is_wildcard = Type == wildcard,
                           value = [CompNode]}
                     | Siblings];
                [{segment, 1, []}] ->
                    [#node{segment = value(Ident, Options),
                ...
                _ ->
                    [#node{segment = value(Ident, Options),
                           is_binding = Type == binding,
                           is_wildcard = Type == wildcard,
                           children = insert(Tl, CompNode, [], Options)}
                     | Siblings]
            end;
        Node ->
            case Tl of
                [] ->
                    case find_comparator(CompNode#node_comp.comparator, Node#node.value) of
                        {error, not_found} ->
                            [Node#node{value = [CompNode | Node#node.value],
                                       is_binding = Type == binding,
                                       is_wildcard = Type == wildcard}
                             | lists:delete(Node, Siblings)];
                        {ok, _NodeComp} ->
...
lookup_segment(Ident,
               Bindings,
               [#node{segment = Segment, is_binding = true} = N | _Tl],
               _WCNode) ->
...
-spec find_comparator(Comparator :: any(), [node_comp()]) ->
                         {ok, Node :: node_comp()} | {error, not_found}.
find_comparator(_, []) ->
    {error, not_found};
...
check_conflicting_nodes(_, _, _, _, []) ->
    false;
...
check_conflicting_nodes(_, _, _, _, _) ->
    false.
...
check_aux([Elem | T], CompNode, Ident, binding) ->
    case check_conflicting_nodes_binding(Elem, CompNode, Ident) of
        {true, Res} ->
            Res;
        _ ->
            check_aux(T, CompNode, Ident, binding)
    end;
check_aux([Elem | T], CompNode, Ident, segment) ->
    case check_conflicting_nodes_segment(Elem, CompNode, Ident) of
        {true, Res} ->
            Res;
        _ ->
            check_aux(T, CompNode, Ident, segment)
    end.

check_conflicting_nodes_segment(#node{value = Value,
                                      segment = Segment,
                                      is_binding = false,
                                      is_wildcard = false} =
                                    Node,
                                CompNode,
                                Ident) ->
    {[C || #node_comp{comparator = C} <- Value, C == CompNode#node_comp.comparator] /= []
     andalso Segment == Ident,
     {duplicate, Node}};
check_conflicting_nodes_segment(#node{is_binding = true} = Node, _, _) ->
    {true, {conflict, Node}};
check_conflicting_nodes_segment(#node{is_wildcard = true} = Node, _, _) ->
    {true, {conflict, Node}}.

check_conflicting_nodes_binding(#node{is_binding = true,
                                      value = Value,
                                      segment = Segment} =
                                    Node,
                                CompNode,
    ...
    Segment == Ident
    andalso {[X
              || #node_comp{comparator = C, value = X} <- Value, C == CompNode#node_comp.comparator]
             /= [],
             {conflict, Node}};
check_conflicting_nodes_binding(#node{is_wildcard = true}, _, _) ->
    true;
check_conflicting_nodes_binding(#node{value = Value} = Node, CompNode, _) ->
    {[X || #node_comp{comparator = C, value = X} <- Value, C == CompNode#node_comp.comparator]
     /= [],
     {conflict, Node}}.
                                            ...
                                            #node{segment = "my",
                                                  ...
                                                  children =
                                                      [#node{segment = "inbox",
                                                             ...
                                                             is_binding = false,
                                                             is_wildcard = false},
                                                       #node{segment = "profile",
                                                             value = [],
                                                             children =
                                                                 [#node{segment = "picture",
                                                                        value =
                                                                            [#node_comp{comparator =
                                                                                            "GET",
                                                                                        value =
...
insert_duplicate_paths_throw_test() ->
    ...
    ?assertException(throw,
                     {duplicated_paths,
                      {"profile", {node_comp, "GET", "TWO"}},
                      {node, "profile", [{node_comp, "GET", "ONE"}], [], false, false}},
                     insert('_', "/profile", "GET", "TWO", B)).
...
insert_non_deterministic_path_test() ->
    ...
    ?assertException(throw,
                     {non_deterministic_paths,
                      {"1", {node_comp, "GET", "TWO"}},
                      {node, "message", [{node_comp, "GET", "ONE"}], [], true, false}},
                     insert('_', "/my/inbox/1", "GET", "TWO", B)).
...
insert_non_deterministic_path2_test() ->
    ...
    ?assertException(throw,
                     {non_deterministic_paths,
                      {"message", {node_comp, "GET", "TWO"}},
                      {node, "1", [{node_comp, "GET", "ONE"}], [], false, false}},
                     insert('_', "/my/inbox/:message", "GET", "TWO", B)).
