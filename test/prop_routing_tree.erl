...
host_tree_aux([[58|Segment]|Rest], Comparator, Value) when length(Rest) == 0 ->
    #node{segment = Segment,
          value = [#node_comp{comparator = Comparator,
                              value = Value}],
          children = [],
          is_binding = true,
          is_wildcard = false
...
path([Char]) when (Char >= $a andalso Char =< $z) orelse
...
prop_insert_non_bindings() ->
    ?FORALL({Host, Path, Comparator, Value, HT}, {any(), string(), any(), any(), #host_tree{}},
        begin
...
prop_combine() ->
    ?FORALL({Host, {Path, Number}, Comparator, Value, HT}, {any(), {string(), pos_integer()}, any(), any(), #host_tree{}},
        begin
